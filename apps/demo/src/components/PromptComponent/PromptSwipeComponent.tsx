...
function PronounceSwipeComponent() {
  ...
  const [promptStatusArray, setPromptStatusArray] = useState<
    { prompt_id: number; status: boolean }[]
  >([]);
  ...
  const handleTouchMove = (e: React.TouchEvent) => {
    if (touchCount === 2) {
      ...
      if (Math.abs(deltaY) > 20) {
        // Threshold for two-finger scroll
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    if (touchCount !== 2) {
      ...
      if (
        ...
        Math.abs(deltaY) > DISTANCE_THRESHOLD
      ) {
        if (deltaY > 0 && index < screens.length - 1) {
          setIndex((current) => Math.min(current + 1, screens.length - 1));
        } else if (deltaY < 0 && index > 0) {
          ...
          sortedPrompts.forEach((prompt) => {
            ...
            if (highlightedTexts.length > 0) {
              ...
              const allArabicHighlights = highlightedTexts
                ...
                .join(" ");
              const firstHighlightAudio =
                highlightedTexts[0].HighlightedTextReferenceAudio;
  ...
  useEffect(() => {
    if (
      promptStatusArray.length > 0 &&
      promptStatusArray.every((item) => item.status)
    ) {
      setPopupOpen(true);
    }
  }, [promptStatusArray]);
    ...
    <div className={`${classes.allSwipeCont}`}>
      ...
      {isPopupOpen && (
        <Popup
          ...
          text2={
            "You've finished lesson 1.1 successfully with a score of ${index + 1}/${screens.length}"
          }
          btnText="Continue"
          btnText2="Next"
