...
interface PromptComponentProps {
  ...
  screens: Screen[];
  setPromptStatusArray: React.Dispatch<
    React.SetStateAction<{ prompt_id: number; status: boolean }[]>
  >;
...
const PromptComponent: React.FC<PromptComponentProps> = ({
  ...
  useEffect(() => {
    ...
    if (currentPromptIndex !== -1) {
      ...
      console.log(
        "No matching prompt status found for promptId:",
        currentPromptId,
      );
    }
    ...
    return (
      ...
      >
        {wordsWithPunctuation.map((word, index) => {
          ...
          const wordIndex = wordsWithoutPunctuation.indexOf(
            cleanWordResult ? cleanWordResult : "",
          );
          const isHighlight = highlightWord
            .toLowerCase()
            .includes(cleanWordResult ? cleanWordResult : "");
          const className = isHighlight
            ? classes.highLightText
            : classes.noPunctuationClass;
  ...
  const highlightTextForTranslation = (
    sentence: string,
    highlightWord: string,
  ) => {
          ...
          >
            {isArabic
              ? payload.highlighted_text_arabic_payload &&
                ...
                : null
              : payload.highlighted_text_latin_payload &&
                  Array.isArray(payload.highlighted_text_latin_payload)
                ? payload.highlighted_text_latin_payload.map(
                    (
    ...
    <>
        ...
        <div className={classes.pronounceTable}>
          <div className={classes.titleContainer}>
            <span className={classes.titleStyle}>
              Pronounce the highlighted words
            </span>
          </div>
          {/* <div className={classes.lineSeperate} /> */}
      ...
      {isPerfect ? (
        <div className={classes.perfectMessage}>
          <span className={classes.swipeSentenceStyle}>
