...
export const SecondeLayout = () => {
    ...
    const [dialectId] = useState<number>(
        location.state?.dialectId ?? parseInt(localStorage.getItem('dialectId') || '0')
    );
    ...
    return (
        <div className='myroot'>
            <Header>
                <img className="back-arrow" src={BackArrow} alt="back-arrow" onClick={() => navigate("/")} />
            ...
            <Main>
                ...
                <div className="favored-themest-container">
                    ...
                    {loading ? (
                        ...
                        themes.slice(0, 4).map(({ theme_id, theme_name, subtitle, logo_url }, index) => (
                            <div
                                key={theme_id}
            ...
            <CustomButton >
                <button type='button' className={classes.actionBtn} onClick={promptNavigate} disabled={selectedThemeId ? false : true}>Yalla!</button>
            </CustomButton>
        </div>
    );
};