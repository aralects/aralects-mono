...
export const GradioProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [gradioClient, setGradioClient] = useState<Client | null>(null);

  useEffect(() => {
    const initializeClient = async () => {
      try {
        const client = await Client.connect("aralects/PronunciationChecker", { hf_token: HF_TOKEN as `hf_${string}` });
