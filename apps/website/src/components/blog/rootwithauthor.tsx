...
const roots: Root[] = [
  { id: '1', name: 'كتب', lexemes: 15, avatar: '/placeholder.svg?height=32&width=32' },
  { id: '2', name: 'قرأ', lexemes: 12, avatar: '/placeholder.svg?height=32&width=32' },
  { id: '3', name: 'علم', lexemes: 20, avatar: '/placeholder.svg?height=32&width=32' },
  { id: '4', name: 'فهم', lexemes: 8, avatar: '/placeholder.svg?height=32&width=32' },
  { id: '5', name: 'درس', lexemes: 10, avatar: '/placeholder.svg?height=32&width=32' },
];
...
export function RootWithAuthor() {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearch(query);
    setFilteredRoots(roots.filter((root) => root.name.toLowerCase().includes(query)));
  };
    ...
    <Card className="w-full max-w-xl">
      ...
      <CardContent>
        <div className="mb-4 flex items-center space-x-2">
          <div className="relative flex-1">
            ...
            <Input placeholder="Search roots..." value={search} onChange={handleSearchChange} className="pl-9" />
            {search && (
              <Button
                variant="ghost"
                size="icon"
        ...
        <ScrollArea className="-mx-6 -mb-6 max-h-[350px] min-h-[350px] overflow-auto px-6 pb-6">
          <div className="space-y-2">
            {filteredRoots.map((root) => (
              <div key={root.id} className="flex items-center justify-between rounded-lg p-2 px-4 hover:bg-accent">
      ...
      {isFilterOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <Card className="w-full max-w-md bg-zinc-950 text-white relative">
            <Button
              variant="ghost"
              size="icon"
